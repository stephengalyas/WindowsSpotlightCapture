...
namespace WindowsSpotlightCapture.Pages
{
    ...
    /// <summary>
    /// A page used to manage Windows Spotlight pictures saved by the user.
    /// </summary>
    public sealed partial class SavedPictures : Page
    {
        /// <summary>
        ...
        private void SavedPictures_Loaded(object sender, RoutedEventArgs e)
        {
            GetImageUris();
        ...
        private async void GetImageUris()
        {
            ...
            // Load each saved image into memory. If the image already exists, catch an exception and move to the next image.
            for (int i=0; i<imageFiles.Count; i++)
            {
                try
                {
                    ...
                    StorageFile imgNewFile = await dirTemp.CreateFileAsync(imgFile.Name, CreationCollisionOption.OpenIfExists);
        ...
        private async void btnOpen_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                StorageFolder sfPics = await StorageFolder.GetFolderFromPathAsync((string)Windows.Storage.ApplicationData.Current.LocalSettings.Values["saveDir"]);
                StorageFile sfOpen = await StorageFile.GetFileFromPathAsync(sfPics.Path + @"\" + images[currIndex].Value.Name);
                await Windows.System.Launcher.LaunchFileAsync(sfOpen);
            }
            catch (Exception exc)
            {
        ...
        /// <summary>
        /// User clicked the Save Image button.o
        /// </summary>
        private async void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                StorageFile sfDelete = await StorageFile.GetFileFromPathAsync((string)Windows.Storage.ApplicationData.Current.LocalSettings.Values["saveDir"] + @"\" + images[currIndex].Value.Name);
                await sfDelete.DeleteAsync(StorageDeleteOption.Default);
            }
            catch (Exception exc)
            {
                success = false;
                Windows.UI.Popups.MessageDialog md = new Windows.UI.Popups.MessageDialog("We could not delete this image. Please try again.\n\nError message: " + exc.Message, (exc.GetType()).Name + " - Could Not Delete Image");
                await md.ShowAsync();
            }
